...
void main() {
  ...
  testWidgets(
    'F3A-W01: wall panel renders a dock, no rail, all labels, and the page',
  ...
  testWidgets(
    'F3A-W02: every dock destination is at least 64 logical px tall',
    (WidgetTester tester) async {
      await tester.pumpWidget(harness(onSelected: (_) {}));
  ...
  testWidgets(
    'F3A-W03: tapping a destination invokes onSelected with its index',
    (WidgetTester tester) async {
      int? selected;
  ...
  testWidgets('F3A-W04: the dock is wrapped in a SafeArea', (
    WidgetTester tester,
  ) async {
    await tester.pumpWidget(harness(onSelected: (_) {}));
